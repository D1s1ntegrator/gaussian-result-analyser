...
def main():
    ...
    paths = generate_paths(args.folder, args.extension, args.nested)
    if len(paths) == 0:
    ...
    for c in range(len(data) - 1):
        if data[c].atomnames[-1] != data[c + 1].atomnames[-1]:
            print("Warning: The arrangement of atoms in the final geometry \
varies between files. Be careful!")
            break
...
3 - root/folder/filename and so on"
    ...
    parser.add_argument(
        "-e", "--extension", default='.log', metavar='\b',
        help="    specify the extension, .log is the default"
        )
    parser.add_argument(
